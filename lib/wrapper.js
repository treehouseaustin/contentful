...
class ContentfulWrapper {
  ...
  constructor(content, config = {}) {
    ...
    _.each(Object.keys(this.fields), (fieldName) => {
      ...
      // Fallback if the field was unable to be parsed (most common with Assets)
      if (!this[fieldName]) {
        this[fieldName] = this.fields[fieldName][this.lang];
      }
    });
  ...
  getSlug() {
    ...
    return [_.get(this.config, `urlMapping.${type}`, type), slug].join('/');
  }
